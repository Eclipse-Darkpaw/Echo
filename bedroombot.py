...
async def help_message(message):
    ...
    if len(command) == 1:
        embed = discord.Embed(title="Gemini Command list",
...
switcher = {'help': help_message, 'ping': ping, 'version_num': version, 'version': version, 'verify': verify,
            'setcode': setcode, 'modmail': modmail, 'quit': end, 'huh': huh, 'kick': kick, 'ban': ban, 'setup': setup,
            'uptime': uptime, 'warn': Mod.warn, 'listwarns': Mod.show_warns, 'removewarn': Mod.remove_warn,
            'ref': nsfw_ref, 'setref': nsfw_set_ref, 'addref': nsfw_add_ref}
