...
# dictionary containing a list of authors who's message were recently deleted, and the number of times messages were
# deleted
deleted_authors = {}
...
whitelist = ['https://discord.gift/',  # legitimate nitro gifting
             ...
             'https://tenor.com', 'https://store.steampowered.com', 'https://twitter.com', 'https://fixupx.com',
             'https://x.com', 'https://open.spotify.com', 'https://www.fxdeviantart.com', 'https://youtube.com',
...
blacklist = ['@everyone', '@here',  # attempting to ping the server
             'https://', 'gift', 'nitro', 'steam', 'free', 'https://discord.gg', 'http://',
...
banlist = ['discorx.gift', 'disords.gift', 'dlsscord-gift.com/', 'discordnitro.fun', 'disordgifts.com',
           ...
           # server invites
           "https://discord.gg/anastasyxxx", "https://discord.gg/t9eKqS8gnG", "https://discord.gg/sexybabe",
           "https://discord.gg/sexgirls", "https://discord.gg/xxxpornx",
...
async def reset_counter(message):
    global deleted_authors
    deleted_authors = {}
    await message.channel.send('Tracker reset')

async def scan_message(message, client):
    global counter
    """
    ...
    :param client: the bot client being used
    :return: None
    """
    ...
    if flags < 2 and bans == 0:
        ...
        if flags >= 3 or bans > 0:
            ...
            try:
                deleted_authors[message.author.id] += 1
            except KeyError:
                deleted_authors[message.author.id] = 1
            ...
            if deleted_authors[message.author.id] <= 3:
                # Automatically suspend users who trigger 3 deletions
                await Moderation.suspend(message=message,
                                         user_id=message.author.id,
                                         reason='Anti scam triggered 3 times',
        ...
        embed.add_field(name='message', value=content.replace('://', ':// '), inline=False)
        ...
        with open(scam_log_path(), 'a') as log:
            ...
            log.write(f'{message.id},{message.created_at},{message.guild.id},{message.author.id},'
                      f'{message.channel.id},{flags},{bans},{lst.replace(",", " - ")},'
