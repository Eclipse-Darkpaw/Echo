...
def add_to_file(file_path: str, content: str):
    """
    Adds new content to the specify file.
    This function utelises file locking to prevent race conditions, this is BLOCKING (up to 5s)!
    If it fails to acquire a lock after 5s, it will timeout.
    ...
    :param file_path: Path of the file to add to
        ...
        with lock.acquire(timeout=5):
            ...
            try:
                with open(file_path, 'a', encoding="utf-8") as file:
                    _logger.debug(f'{file_path} opened for writing')

                    file.write(content)
                    file.flush()
            except:
                _logger.exception(f'Unexpected error while adding to: {file_path}')
                raise RuntimeError(f'Unexpected error while adding to: {file_path}')
            finally:
                _logger.debug(f'Lock released, for: {file_path}')
